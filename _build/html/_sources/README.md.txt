...
```mermaid
graph LR;
    A(Language Library) --- B(Wallet daemon);
    C(REST API) --- B;
    B --- D(Hub);
```
...
participant Wallet daemon
participant Hub
Language Library -> Wallet daemon: GET /transfers/0x123...aBC/32
Wallet daemon -> Wallet daemon: Creates the transfer
Wallet daemon -> Hub: Send transfer
Wallet daemon --> Hub: Any updates?
Hub -> Wallet daemon: Update: Transfer confirmed
Language Library --> Wallet daemon: GET /transfers?status=pending
Wallet daemon -> Language Library: []
```

1. The user ask the daemon to perform a transfer of 32 wei
...
Liquidity language library is a convenient way to communication with the daemon. It wraps the provided REST API using language specific feature. For now, the language library is only available for Node.JS. If you have build an implementation for a language you love, you can submit an issue on [liquidity sdk repo](https://github.com/liquidity-network/liquidity-daemon).
...
Alternatively, you can download it from [here](https://github.com/liquidity-network/liquidity-daemon/archive/master.zip)
...
This command will start your wallet daemon. It can take some time for it to synchronise with the hub, especially at first launch.

#### 4. Celebrate :tada:

You're done with the installation! The SDK is self-hosted on your machine and is accessible under `localhost:3600`.

You can try accessing [https://localhost:3600/wallet](https://localhost:3600/wallet) where you should see the current state of your wallet.
...
If you spot any issues, please post an issue on our [github repository](https://github.com/liquidity-network/liquidity-daemon)
...
```http
POST /transfers/:recipient/:amount
```
...
```http
POST /transfers/0x627306090abaB3A6e1400e9345bC60c78a8BEf57/1
```
...
```http
POST /invoices/1?recipient=0x627306090abaB3A6e1400e9345bC60c78a8BEf57
```
...
**Privacy**: no private key within the daemon, messages should be signed client side. It would allow a liquidity managed daemon

**Registration**: If the address is not yet registered, the daemon should go through the process